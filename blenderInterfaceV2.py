...
def build_gens():
    ...
    print(f"Building Gen: {removeGen + 1}")
    
    for b in branchList:
        if not b.built:
            b.make_branch()
            
        if b.generation <= removeGen:
            b.make_sphere()
            removeList.append(b)
    ...
    print(f"Removing Gen:{removeGen} data")
    for b in removeList:
        branchList.remove(b)
        if b.succID in touchingBranches:
            touchingBranches.pop(b.succID)        
        deleter(b.cylinder)
...
def clean():
    ...
    print(f"Gen:{removeGen} FINAL CLEAN")
    print(f"Gen:{removeGen +1 } FIRST CLEAN")
...
with open(path, "r", newline='') as data:
    ...
    for row in info:
        ...
        if currentGen != rowData[3]:
            currentGen = rowData[3]
            removeGen = currentGen - 2
            if removeGen in genBranches:
                build_gens()
        ...
        if rowData[3] <= max_gen:
            ...
            if currentGen not in genBranches:
                genBranches[currentGen] = [rowData[0],]
            else:
                genBranches[currentGen].append(rowData[0])
